...
/* STEP 10 - Declare a varaible app_callbacks of type my_lbs_cb and initiate its members to the
 * applications call back functions we developed in steps 8.2 and 9.2. */
...
int main(void)
{
	...
	/* STEP 11 - Pass your application callback functions stored in app_callbacks to the MY LBS
	 * service */
	err = my_lbs_init(&app_callbacks);
	if (err) {
		LOG_ERR("My LBS service init failed (err %d)\n", err);
		return -1;
	}
