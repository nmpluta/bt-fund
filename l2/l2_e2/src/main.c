...
#define COMPANY_ID_CODE 0x0059 /* Nordic Semiconductor Identifier */
...
static void button_changed(uint32_t button_state, uint32_t has_changed)
{
	...
	if (has_changed & button_state & USER_BUTTON) {
		...
		/* Update the advertising data with the new button press count */
		err = bt_le_adv_update_data(ad, ARRAY_SIZE(ad), sd, ARRAY_SIZE(sd));
		if (err) {
			printk("Failed to update advertising data (err %d)\n", err);
		} else {
			printk("Advertising data updated. Button pressed %d times\n",
			       adv_mfg_data.number_press);
		}
	}
}
