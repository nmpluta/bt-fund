...
#define LBS_SERVICE_UUID BT_UUID_128_ENCODE(0x00001523, 0x1212, 0xefde, 0x1523, 0x785feabcd123)
...
static const struct bt_data sd[] = {
	/* STEP 3.2.2 - Include the 16-bytes (128-Bits) UUID of the LBS service in the scan response
	   packet */
	BT_DATA_BYTES(BT_DATA_UUID128_ALL, LBS_SERVICE_UUID),
...
int main(void)
{
	...
	if (err) {
		LOG_ERR("Invalid Bluetooth address (err %d)\n", err);
		return -1;
	}

	err = bt_id_create(&addr, NULL);
	if (err) {
		LOG_ERR("Create new identity failed (err %d)\n", err);
		return -1;
	}
